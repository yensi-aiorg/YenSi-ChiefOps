...
class TestUploadFile:
    ...
    async def test_upload_file(self, async_client: AsyncClient, test_db):
        """POST /api/v1/ingestion/upload should accept valid files and create a job."""
...
class TestListJobs:
    ...
    async def test_list_jobs(self, async_client: AsyncClient, test_db):
        """GET /api/v1/ingestion/jobs should return a paginated list of jobs."""
...
class TestGetJobDetail:
    ...
    async def test_get_job_detail(self, async_client: AsyncClient, test_db):
        """GET /api/v1/ingestion/jobs/{job_id} should return the full job detail."""
        # Create a job
        csv_content = b"Issue Key,Summary,Status\nPROJ-1,Task,Done\n"
        ...
        response = await async_client.get(f"/api/v1/ingestion/jobs/{job_id}")
    ...
    async def test_get_job_detail_not_found(self, async_client: AsyncClient):
        ...
        response = await async_client.get(
            f"/api/v1/ingestion/jobs/{generate_uuid()}"
        )
